...
class GroundingDataset(torch.utils.data.Dataset):
    ...
    def get_image_url(self, image_url):
        if image_url.startswith('/blob/v-lqinghong/'):
            key = "images"
            image_url = image_url[image_url.find(key)+len(key)+1:]
            return image_url
        else:
            return image_url
    ...
    def get_sample(self, idx):
        ...
        if 'img_url' in item.keys():
            img_url = self.get_image_url(item["img_url"])
            image_path = os.path.join(self.IMG_DIR, img_url)
            
            print("read image: ", image_path, img_url, self.IMG_DIR)

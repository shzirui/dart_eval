...
from tongui.data.template import batch_add_answer, mind2web_to_qwen
...
def get_history(sample, num_history, interleaved_history="tttt"):
    ...
    action_history = []
    action_prefix = []
    for i, (step, step_repr) in enumerate(
        zip(step_history[-num_history:], repr_history[-num_history:]), start=1
    ):
...
class Mind2WebDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.IMG_DIR = os.path.join(self.base_image_dir, "ming2web_images")
        with open(os.path.join(META_DIR, "{}.json".format(json_data))) as f:
            self.json_data = json.load(f)

        self.samples_per_epoch = args_dict.get("samples_per_epoch", 1)

        # META_DIR = os.path.join(self.base_image_dir)
        self.IMG_DIR = os.path.join(self.base_image_dir, "ming2web_images")
        with open(os.path.join(META_DIR, "{}.json".format(json_data))) as f:
            self.json_data = json.load(f)
        self.version = args_dict.get("version", "v1")
        # json_data_filter = []
        # if self.version == "v2":
        #     for sample in self.json_data:
        #         if len(sample['step_history']) == 0:
        #             json_data_filter.append(sample)

        # self.json_data = json_data_filter
        ...
        print(
            f"Dataset: Mind2Web; Split: {json_data}; # samples: {len(self.json_data)}"
        )
    ...
    def get_history_qwen(
        ...
        for i, step in enumerate(step_history[-num_history:]):
            action = get_answer(sample, step, sample["repr_history"][i])
            thoughts = step["thoughts"] if "thoughts" in step else None
            max_pixels = max(
                self.min_pixels, self.max_pixels * decay_factor ** (num_history - i)
            )
            if interleaved_history == "vvtt":
        ...
        while i <= L - (vis_end_len + vis_start_len):
            if torch.equal(input_ids[0, i : i + vis_end_len], torch.tensor(vis_end)):
                for j in range(i + vis_end_len, L - vis_start_len + 1):
                    if torch.equal(
                        input_ids[0, j : j + vis_start_len], torch.tensor(vis_start)
                    ):
                        labels[0, i + vis_end_len : j] = input_ids[
                            0, i + vis_end_len : j
                        ]
                        i = j + vis_start_len - 1
                        break
    ...
    def get_sample(self, idx):
        ...
        if self.interleaved_history in ["vvvv", "vvtt", "ttvv", "vtvt", "tvtv"]:
            image_list = self.append_history_image(
                item, self.num_history, image_list, url_only=True
            )
        ...
        )  # if item['step_history'] != [] else image_list

        item["anno_id"] = idx
        item["answer"] = answer_dict

        source = mind2web_to_qwen(task, action_history, None, version=self.version)
        item["source"] = source
        ...
        data_dict_qa, answer = batch_add_answer(
            data_dict_q, answer_dict, self.processor
        )
        if self.num_turn > 1:
            data_dict_qa["labels"] = self.activate_history_labels(
                data_dict_qa["input_ids"],
                data_dict_qa["labels"],
                self.vis_end,
                self.vis_start,
            )

...
def load_screenspot_data(
    ...
    base_image_dir = os.path.join(dataset_dir, "ScreenSpot-Pro")
    meta_dir = os.path.join(base_image_dir, "annotations")
...
def main():
    ...
    # MODEL = "qwen2.5-7b"
    MODEL = "tongui-7b"
    ENDPOINT = "http://hgx-hyperplane06:8000/v1"
    ...
    for idx, item in tqdm(enumerate(json_data), total=len(json_data)):
        ...
        gt_bbox_xyxy = [gt_bbox[0], gt_bbox[1], gt_bbox[2], gt_bbox[3]]
        ...
        if pred_point:
            ...
            split = item.get('group', 'unknown')
            data_type = item.get('ui_type', 'unknown')
